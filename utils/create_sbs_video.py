...
def create_combined_video(
    video_path1, video_path2, text1, text2, output_path="combined_video.mp4"
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create SBS video from SAM2 and exp results"
    )
    ...
    parser.add_argument(
        "--output_path", default="combined_video.mp4", help="Path to save result"
    )

    args = parser.parse_args()

    create_combined_video(
        args.sam2_result, args.exp_result, "SAM2", args.exp_name, args.output_path
    )
