...
# import torchvision.transforms as transforms # Заменяем на albumentations
import cv2
import numpy as np
...
import albumentations as A  # Добавлено
from albumentations.pytorch import ToTensorV2  # Добавлено

# Предполагается, что эти модули находятся в вашем проекте
from modeling.model import TemporalFusionModule, config_tiny_mamba_bbox

# Если вы используете LightningModule для загрузки, раскомментируйте:
from trainer import TemporalFusionTrainer

# --- Константы и Настройки ---
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# Важно: Используйте те же трансформации, что и при обучении!
# Пример с albumentations (адаптируйте под ваши реальные трансформации):
IMG_TRANSFORM = A.Compose(
    [
        # Укажите тот же размер, что использовался при обучении!
        A.Resize(256, 256),
        # Используйте ту же нормализацию, что и при обучении
        A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ToTensorV2(),  # Конвертирует numpy HWC -> torch CHW и применяет нормализацию
    ]
)
...
def normalize_bbox(bbox, img_w, img_h):
    ...
    norm_w = w / img_w
    norm_h = h / img_h
    return torch.tensor([norm_x, norm_y, norm_w, norm_h], dtype=torch.float32).clamp(
        0.0, 1.0
    )
...
def run_inference(checkpoint_path, input_dir, output_video_path, initial_bbox_str):
    ...
    with torch.no_grad():
        for t in tqdm(range(1, num_frames), desc="Инференс"):
            ...
            transformed_frames = []
            for frame_pil in current_pil_frames:
                # Конвертируем PIL Image в numpy array (RGB)
                frame_np = np.array(frame_pil)
                # Применяем albumentations transform
                transformed = IMG_TRANSFORM(image=frame_np)
                transformed_frames.append(transformed["image"])  # Извлекаем тензор

            # Стекуем тензоры [C, H, W] в один тензор [T, C, H, W]
            input_frames_tensor = torch.stack(transformed_frames)
            ...
            predicted_bbox_norm_t = predicted_bboxes_norm_all[
                0, -1, :
            ].cpu()  # Shape: (4,)

            # 5. Денормализация и сохранение
            predicted_bbox_pixel_t = denormalize_bbox(
                predicted_bbox_norm_t.numpy(), img_w, img_h
            )
    ...
    out_video = cv2.VideoWriter(
        output_video_path, fourcc, 25, size
    )  # 25 FPS - можно настроить
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Инференс модели TemporalFusionModule для трекинга."
    )
    parser.add_argument(
        "--checkpoint", type=str, required=True, help="Путь к файлу чекпоинта (.ckpt)."
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        required=True,
        help="Директория с кадрами видео (jpg, png).",
    )
    parser.add_argument(
        "--output_video",
        type=str,
        default="output_tracking.mp4",
        help="Путь для сохранения выходного видео.",
    )
    parser.add_argument(
        "--initial_bbox",
        type=str,
        required=True,
        help="Начальный bounding box для первого кадра в формате 'x,y,w,h' (пиксельные координаты).",
    )
