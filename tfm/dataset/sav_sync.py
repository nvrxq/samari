...
class SyncAVDataset(Dataset):
    ...
    def _load_annotations(self, file_path: str) -> list[np.ndarray] | None:
        ...
        for frame_data in annotations["masklet"]:
             if obj_id < len(frame_data) and frame_data[obj_id] is not None:
                 try:
                     first_valid_mask_shape = mask_util.decode(frame_data[obj_id]).shape[:2]
                     break
                 except Exception as e:
                     logger.error(f"Error decoding mask for object {obj_id} in {mask_path}: {e}")
                     return None # Cannot proceed without a valid mask shape

        if first_valid_mask_shape is None:
            logger.error(f"Could not find any valid mask for object {obj_id} in {mask_path} to determine shape.")
            return None
        ...
        for frame_data in annotations["masklet"]:
             ...
             if obj_id >= len(frame_data) or frame_data[obj_id] is None:
                 ...
                 logger.debug(f"Object {obj_id} missing in a frame for {mask_path}, using zero mask.")
             else:
                 try:
                     ...
                     if decoded_mask.shape[:2] != (h, w):
                         logger.warning(f"Inconsistent mask shape for object {obj_id} in {mask_path}. Expected {(h,w)}, got {decoded_mask.shape[:2]}. Using zero mask.")
                         masks.append(np.zeros((h, w), dtype=np.uint8))
                     else:
                         masks.append(decoded_mask)
                 except Exception as e:
                     logger.error(f"Error decoding mask for object {obj_id} in frame for {mask_path}: {e}. Using zero mask.")
                     masks.append(np.zeros((h, w), dtype=np.uint8))
        ...
        if not subsampled_masks:
             logger.warning(f"Subsampling resulted in empty mask list for {mask_path} with rate {self.annot_sample_rate}")
             return None
    ...
    def __getitem__(self, index):
        ...
        assert len(frames) == len(annotations), f"Number of frames and annotations do not match after subsampling. Frames: {len(frames)}, Annotations: {len(annotations)}"
        ...
        if len(frames) != len(annotations):
             logger.error(f"Number of frames ({len(frames)}) and annotations ({len(annotations)}) "
        ...
        if num_available_frames < self.frames_per_sample or num_available_annots < self.frames_per_sample:
            logger.info(f"Skipping index {index}: Not enough frames/annotations after subsampling. "
                           f"Got {num_available_frames} frames, {num_available_annots} annots. "
                           f"Need at least {self.frames_per_sample} of each.")
            raise ValueError(f"Skipping index {index}: Not enough frames/annotations. Frames len: {num_available_frames}, Annots len: {num_available_annots}")

        # --- Sampling ---
        # Select a starting point ensuring we have T frames and T masks
        max_start = num_available_frames - self.frames_per_sample # Both are already subsampled

        if max_start < 0:
             raise ValueError(f"Cannot determine valid start for index {index} with {num_available_frames} frames/annotations for sample size {self.frames_per_sample}")
        ...
        selected_frames = frames[bos:eos] # Shape (T, H, W, C) - RGB from sav_decode_video
        ...
        frames_tensor = torch.tensor(frames_np, dtype=torch.float32).permute(0, 3, 1, 2) / 255.0
...
if __name__ == "__main__":
    ...
    if not os.path.exists(dummy_video_path):
        ...
        total_video_frames = (frames_needed_for_sample + 5) * dummy_annot_sample_rate # Add buffer
        ...
        for i in range(total_video_frames):
            ...
            cv2.rectangle(frame, (x, y), (x + 10, y + 10), (0, 255, 0), -1) # Use BGR for writing
            writer.write(frame)
        writer.release()
        ...
        for i in range(total_annot_frames):
            ...
            x = min(max(0, int(original_frame_idx * width / total_video_frames)), width - 10)
            y = min(max(0, int(original_frame_idx * height / total_video_frames)), height - 10)
...
Label shape: {batch['label'].shape}, dtype: {batch['label'].dtype}, range: [{batch['label'].min():.2f}, {batch['label'].max():.2f}]""")
            ...
            assert batch['masks'].shape == (test_frames_per_sample - 1, 1, 64, 64) # T-1
